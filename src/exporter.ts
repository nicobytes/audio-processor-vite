function _writeStringToArray(aString: string, targetArray: Uint8Array, offset: number) {
  for (let i = 0; i < aString.length; ++i) {
    targetArray[offset + i] = aString.charCodeAt(i);
  }
}

function _writeInt16ToArray(aNumber: number, targetArray: Uint8Array, offset: number) {
  aNumber = Math.floor(aNumber);
  targetArray[offset + 0] = aNumber & 255; // byte 1
  targetArray[offset + 1] = (aNumber >> 8) & 255; // byte 2
}

function _writeInt32ToArray(aNumber: number, targetArray: Uint8Array, offset: number) {
  aNumber = Math.floor(aNumber);
  targetArray[offset + 0] = aNumber & 255; // byte 1
  targetArray[offset + 1] = (aNumber >> 8) & 255; // byte 2
...
function _writeAudioBufferToArray(audioBuffer: AudioBuffer, targetArray: Uint8Array, offset: number, bitDepth: 32 | 16) {
  let index = 0;
  let channel = 0;
...
function createWaveFileBlobFromAudioBuffer(audioBuffer: AudioBuffer, as32BitFloat: boolean) {
  // Encoding setup.
...
export default createWaveFileBlobFromAudioBuffer;
